(function(){
    ...
    if (saveBudgetBtn){
        saveBudgetBtn.addEventListener('click', ()=>{
            ...
            localStorage.setItem('finedu_budget', JSON.stringify({income,savePct}));
            alert('Rencana anggaran tersimpan di perangkat ini');
        });
    }
    if (clearBudgetBtn){
        clearBudgetBtn.addEventListener('click', ()=>{
            localStorage.removeItem('finedu_budget');
            budgetResult.innerHTML = '';
            alert('Rencana anggaran dihapus');
        });
    }
    const storedBudget = localStorage.getItem('finedu_budget');
    ...
    const renderExpenses = (items=[]) =>{
     ...
     $$('#expenseList button[data-i]').forEach(btn=>{
        btn.addEventListener('click', ()=>{
            ...
            items.splice(i,1);
            localStorage.setItem('finedu_expenses', JSON.stringify(items));
    ...
    if (expenseForm){
        const items = JSON.parse(localStorage.getItem('finedu_expenses') || '[]');
        renderExpenses(items);
